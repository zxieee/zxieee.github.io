...
function GifMock({ media, variant = '' }) {
  ...
  // First-frame poster lives in assets/<basename>-poster.png. Shown until the
  // video starts playing, and acts as a fallback on mobile browsers that
  // block autoplay.
  const poster = isVideo ? `assets/${media.replace(/\.(mp4|mov|webm)$/i, '-poster.png')}` : undefined;
  return (
...
function VideoMock({ media }) {
  const poster = `assets/${media.replace(/\.(mp4|mov|webm)$/i, '-poster.png')}`;
  return (
...
Object.assign(window, { MeshBG, TopNav, Marquee, ProjectMock, GifMock, VideoMock, DuoMock, DiagramMock });